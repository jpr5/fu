...
## PatchLoader because order is important and we depend on it (PL can come after
## our load).  Another reason is because we don't want DB-specific patches
...
# is there left to do?  At least the goddamn thing works.a
...
::DataMapper::Model.module_eval      { include ::FU::Mixins::Paginator::ClassMethods }
::DataMapper::Collection.module_eval { include ::FU::Mixins::Paginator::ClassMethods }


# MySQL kills idle connections.  Sometimes there isn't much website traffic, so
# we end up exceeding the db-configured idle timeout.  We need to keep that
...
# to keepalive the connection.  Arbitrarily picking 30 minutes as an interval.
