# FU Session Store Middleware (Rack-based)
#
# History
#
...
module FU
    class SessionStore
        ...
        class Rack < ::Rack::Session::Abstract::ID
            ...
            # use FU::SessionStore::Rack, :collection => "session", ..
            ...
            def generate_sid
                loop do
                    sid = ::FU::Utils::RefCode.generate(@sid_length)
            ...
            def with_lock(env, default=nil)
                ...
                $LOG.error(e) { "FU SessionStore failed" }
                default
            ensure
