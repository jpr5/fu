...
moduleFU 
    class DB < Module
        ...
        # time.
        def setup(from_dir = FUROOT, repos = :all, force = false)
            configure(from_dir + "/config/database.yml", repos, force)
            load_models(from_dir + '/app/models', true)

            # Newer MySQL enables sql_mode ONLY_FULL_GROUP_BY on when ANSI is
            # set.  This normally isn't a problem for us; however some of the
            # old nasty-ass PHP SQL queries aren't compatible so we disable it.
            # DataObjects sets ANSI mode inside the driver, so we explictly
            # override it afterwards (here).
            $DB.execute("set session sql_mode = 'NO_BACKSLASH_ESCAPES,NO_DIR_IN_CREATE," +
        ...
        def configure(input = FUROOT + "/config/database.yml", new_repos = :all, force = false)
            # Support both a Hash and a String.  Default is to support old
            # convention (input is a filename string).
            newconf = input
            newconf = ::FU::Config.load(newconf) if newconf.kind_of? String
        ...
        def load_models(from_dir = FUROOT + '/app/models', validate = true)
...
require 'fu/db/patches'
require 'fu/db/repository'
require 'fu/db/property'
require 'fu/db/logging'
require 'fu/db/fixtures'
...
# forever. :-/
$DB = ::FU::DB.instance
$DB.send(:include, ::FU::DB::Exceptions)
