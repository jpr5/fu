...
module FU
    class Config < Hash
        ...
        def initialize(file = nil, env = nil)
            ...
            @env    = (env || ENV['FU_ENV'] || ENV['RAILS_ENV'] || ENV['RACK_ENV'] || 'development').to_sym
            @file   = file
            @config = {}
...
# By including this file, we auto-load the app config for you.  We *don't*
# freeze it so that you can override it later, if need be.  Just don't be stupid
# about it.
$CONFIG = ::FU::Config.load(Dir.pwd / 'config/app.yml')
