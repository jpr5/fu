...
module FU
    ...
    class Log < ::Logger
        ...
        {
            ::Logger::UNKNOWN => ::FU::Syslogger::LOG_WARNING,
            ::Logger::DEBUG   => ::FU::Syslogger::LOG_DEBUG,
            ::Logger::INFO    => ::FU::Syslogger::LOG_INFO,
            ::Logger::WARN    => ::FU::Syslogger::LOG_WARNING,
            ::Logger::ERROR   => ::FU::Syslogger::LOG_ERR,
            ::Logger::FATAL   => ::FU::Syslogger::LOG_EMERG,
        }
        ...
        # FU::Log#configure expects to be passed a hash that would be typically
        # be under "logging:".  We can get called as a (re)configure, so could
        # be either opening or closing logs.

        def configure(config)
            config = config.is_a?(Hash) ? config.dup : ::FU::Config.load(config)[:logging]
            ...
            config.each_pair do |k, v|
                ...
                when :level then
                    ...
                    @level = Thread.current[:fu_log_level] = Thread.main[:fu_log_level] = v
                ...
                when :syslog then
                    ...
                    unless facility.to_sym.among?(FU::Syslogger.constants)
                        next self.error "unknown logging facility #{v.inspect}"
                    end

                    @syslogger = ::FU::Syslogger.new({
                            :facility => facility,
                            :host     => host,
        ...
        def level
            Thread.current[:fu_log_level] || Thread.main[:fu_log_level] || 0
        end
        ...
        def add(severity, message = nil, progname = nil)
            ...
            self << formatter.call(format_severity(severity), Time.now, progname, message.dup)
        ...
        def with_level(level)
            ...
            Thread.current[:fu_log_level] = level
            return yield
        ensure
            Thread.current[:fu_log_level] = save_level
        end
