...
deps_system = []  # interpolated into config/deps.rb
deps_fu     = []  # interpolated into config/deps.rb
deps_local  = []  # interpolated into config/deps.rb
...
task 'skeleton:base' do
    ...
    links << [ '/lib', COMMON_DIR + '/ruby',       'fu' ]
    links << [ '/lib', COMMON_DIR + '/ruby/tasks', 'tasks' ]

    deps_system += %w[  ]
    deps_fu     += %w[ utils log config ]
end
...
task 'skeleton:sinatra' do
    ...
    deps_fu     += %w[ sinatra rails/helpers ]
    deps_local  += %w[ server ]
...
task 'skeleton:finalize' do
    ...
    FU_DEPS     = deps_fu.map     { |d| "require 'fu/#{d}'\n" }
