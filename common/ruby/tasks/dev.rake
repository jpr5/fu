namespace :dev do
    ...
    def each_repo_do
        ...
        db_conf = ::FU::Config.load(db_conf_file)
    ...
    def mysql_dump(db_name, dump_name)
        ...
        run_or_die("mysqldump -u fuadmin #{db_name} | xz > #{BACKUP_DIR}#{dump_name}/#{db_name}.sql.xz")
    end

    def mysql_restore(db_name, dump_name)
        puts "Restoring mysql db #{db_name} from #{dump_name}"
        ensure_has_dump_dir(dump_name)
        run_or_die("xz -dc #{BACKUP_DIR}#{dump_name}/#{db_name}.sql.xz | mysql -u fuadmin #{db_name}")
    end
